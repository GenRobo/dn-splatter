...
class DNSplatterModel(SplatfactoModel):
    ...
    def cull_gaussians(self, extra_cull_mask: Optional[torch.Tensor] = None):
        """
        Delete gaussians with under a certain opacity threshold.

        Args:
            extra_cull_mask: Optional mask indicating additional gaussians to cull.
        """
        ...
        if self.step > self.config.refine_every * self.config.reset_alpha_every:
            # cull huge ones
            toobigs = (
                torch.exp(self.scales).max(dim=-1).values > self.config.cull_scale_thresh
            ).squeeze()
    ...
    def split_gaussians(self, split_mask, samps):
        """Split gaussians that are too large (original DN-Splatter behavior)."""
    ...
    def dup_gaussians(self, dup_mask):
        """Duplicate gaussians that are too small (original DN-Splatter behavior)."""
    ...
    def remove_from_optim(self, optimizer, deleted_mask, new_params):
        """Remove entries from a torch optimizer's state for a pruned gaussian mask."""
    ...
    def dup_in_optim(self, optimizer, dup_mask, new_params, n=2):
        """Expand a torch optimizer's state for newly duplicated gaussians."""
        param = optimizer.param_groups[0]["params"][0]
        param_state = optimizer.state[param]
        if "exp_avg" in param_state:
            repeat_dims = (n,) + tuple(
